// POST /api/subscription/webhook — Workers port of
...
import { resolvePlan } from '../../lib/plan.js';
...
export async function subscriptionWebhookHandler(c) {
  if (c.req.method !== 'POST') return c.json({ error: 'Method not allowed' }, 405);

  const RC_WEBHOOK_SECRET = c.env.REVENUECAT_WEBHOOK_SECRET;
  ...
  if (!event?.type || !event?.app_user_id) {
    return c.json({ error: 'Missing event.type or event.app_user_id' }, 400);
  }
  ...
  try {
    if (!(await authUserExists(c.env, userId))) {
    ...
    await upsertSubscription(c.env, { userId, status, plan, periodType, expiresAt, productId });
...
async function authUserExists(env, userId) {
...
async function upsertSubscription(env, { userId, status, plan, periodType, expiresAt, productId }) {
