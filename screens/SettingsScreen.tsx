...
const PRIVACY_URL =
  Constants.expoConfig?.extra?.privacyPolicyUrl ?? "https://tradeready.app/privacy";
  ...
  async function handleStripeConnect() {
    ...
    try {
      ...
      Alert.alert(
        ...
        (err as Error).message || "Could not start Stripe onboarding."
      );
    } finally {
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert(
                "Error",
                (err as Error).message || "Could not disconnect Stripe account."
              );
            } finally {
      ...
      >
        ...
        <View style={styles.card}>
          ...
          <Field
            ...
            value={s.paymentNotes}
            onChangeText={(v) => update("paymentNotes", v)}
            multiline
        ...
        <View style={styles.card}>
          <Field
            label="Your hourly labor rate ($)"
            value={String(s.laborRate || "")}
            onChangeText={(v) => update("laborRate", parseFloat(v) || 0)}
          ...
          <Field
            ...
            value={String(s.materialMarkup || "")}
            onChangeText={(v) => update("materialMarkup", parseFloat(v) || 0)}
            keyboardType="decimal-pad"
            colors={colors}
            shadow={shadow}
          />
          <Field
            label="Overhead % (insurance, truck, tools)"
            value={String(s.overheadPercent || "")}
            onChangeText={(v) => update("overheadPercent", parseFloat(v) || 0)}
            keyboardType="decimal-pad"
            colors={colors}
            shadow={shadow}
          />
          <Field
            label="Profit margin %"
            value={String(s.marginPercent || "")}
            onChangeText={(v) => update("marginPercent", parseFloat(v) || 0)}
            keyboardType="decimal-pad"
            colors={colors}
            shadow={shadow}
          />
          <Field
            ...
            value={String(s.minimumJobFee || "")}
            onChangeText={(v) => update("minimumJobFee", parseFloat(v) || 0)}
            keyboardType="decimal-pad"
            colors={colors}
            shadow={shadow}
          />
          <Field
            label="Emergency/after-hours multiplier (e.g. 1.5 = 50% extra)"
            value={String(s.emergencyMultiplier || "")}
            onChangeText={(v) => update("emergencyMultiplier", parseFloat(v) || 1)}
            keyboardType="decimal-pad"
            colors={colors}
            shadow={shadow}
          />
        ...
        <Text style={styles.ruleSubtitle}>
          Used to estimate your tax deduction from logged trips (Money → Mileage). Set this to the
          standard mileage rate for your tax year.
        </Text>
        <View style={styles.card}>
          <Field
            label="Mileage rate ($ per mile)"
            value={String(s.mileageRate ?? 0.7)}
            onChangeText={(v) => update("mileageRate", parseFloat(v) || 0)}
            keyboardType="decimal-pad"
            colors={colors}
            shadow={shadow}
          />
        ...
        <View style={styles.providerGrid}>
          {PROVIDERS.map((p) => (
            ...
            >
              <Text
                style={[styles.providerLabel, s.provider === p.id && styles.providerLabelActive]}
              >
                {p.label}
              </Text>
            </TouchableOpacity>
        ...
        {s.provider === "stripe" ? (
            ...
            {stripeStatus === null ? (
                ...
                <View style={styles.stripeConnectedRow}>
                  ...
                  <Text style={styles.stripeConnectedLabel}>
                    {stripeStatus.details_submitted
                      ? stripeStatus.display_name
                        ? `Connected — ${stripeStatus.display_name}`
                        : "Connected"
              ...
              <>
                <Text style={styles.providerHint}>
                  Connect your Stripe account to generate payment links for your customers. Payments
                  go directly to your Stripe account.
                </Text>
                <TouchableOpacity
          ...
          <View style={styles.card}>
            ...
            <Text style={styles.keyNote}>
              Stored only on your device. Never share it with anyone.
            </Text>
          </View>
        ) : null}
        ...
        <View style={styles.card}>
          <Text style={styles.providerHint}>
            Groq API key — powers the AI chat tab (estimates, advice, invoice messages). Get a free
            key at console.groq.com — no billing required.
          </Text>
          <TextInput
            style={styles.input}
            value={s.groqKey}
        ...
        <View style={[styles.card, { marginTop: spacing.sm }]}>
          <Text style={styles.providerHint}>
            Anthropic (Claude) API key — used for AI-generated invoice outreach messages. Get one at
              ...
              >
                <Text
                  style={[
                    ...
                    preference === opt.key && styles.providerLabelActive,
                  ]}
                >
                  {opt.label}
                </Text>
        ...
        <Text style={styles.ruleSubtitle}>
          Get notified when an invoice is this many days past due:
        </Text>
        {s.rules.map((rule, i) => (
        ...
        {s.reviewRequestEnabled && (
          <>
            ...
            <View style={styles.card}>
              ...
              />
              <Text style={styles.keyNote}>
                Use {"{businessName}"}, {"{customerName}"}, and {"{googleReviewLink}"} as
                placeholders.
              </Text>
            </View>
          </>
        ...
        <View style={styles.card}>
          {isTrialing ? (
            <View style={styles.subStatusRow}>
              ...
              <Text style={[styles.subStatusLabel, { color: colors.warning }]}>
                Free trial active
              </Text>
            </View>
          ) : isSubscribed ? (
            <View style={styles.subStatusRow}>
              ...
              <Text style={[styles.subStatusLabel, { color: colors.success }]}>
                TradeReady Pro — active
              </Text>
            </View>
          ) : (
            <Text style={styles.providerHint}>
