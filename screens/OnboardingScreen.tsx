...
export default function OnboardingScreen({ onComplete }: { onComplete: () => void }) {
  ...
  function canContinue(): boolean {
    if (step === 1)
      return form.businessName.trim().length > 0 && form.contactName.trim().length > 0;
    return true;
  }
  ...
  async function finish() {
    ...
    posthog.capture("onboarding_completed", {
      trade: form.trade,
      data_choice: form.dataChoice,
      has_region: !!form.region.trim(),
    });
    onComplete();
  }
    ...
    <SafeAreaView style={styles.container} edges={["top", "bottom"]}>
      ...
      >
        ...
        >
          ...
          {step === 2 && (
            <StepTrade form={form} update={update} touched={touched} markTouched={markTouched} />
          )}
  ...
  return (
    <View style={styles.stepContent}>
      ...
      <Text style={styles.welcomeBody}>
        Set up your account in 2 minutes and start managing jobs, invoices, and customers — all in
        one place.
      </Text>
...
function StepTrade({
  ...
  touched,
  markTouched,
}: StepProps & { touched: Record<string, boolean>; markTouched: (field: string) => void }) {
  const { colors, shadow } = useTheme();
  const styles = useMemo(() => createStyles(colors, shadow), [colors, shadow]);
...
function RateSuggestion({
  ...
  return (
    <>
      ...
      {touched.laborRate &&
        (isNaN(parseFloat(form.laborRate)) ||
          parseFloat(form.laborRate) < 10 ||
          parseFloat(form.laborRate) > 500) && (
          <Text style={styles.warningText}>
            This rate seems unusual — double-check before continuing.
          </Text>
        )}
    ...
    <View style={styles.doneContent}>
          ...
          <View style={styles.notifResult}>
            <Text style={styles.notifResultText}>
              {notifGranted
                ...
                : "Notifications off — enable in device Settings any time."}
            </Text>
          </View>
        ) : (
      ...
      <View style={styles.stripeInfoCard}>
        <View style={styles.notifHeader}>
          ...
          <View style={styles.notifText}>
            ...
            <Text style={styles.notifDesc}>
              Connect your Stripe account in Settings → Payment Processor to send payment links with
              your invoices.
            </Text>
          </View>
        </View>
      ...
      {actions.length > 0 ? (
        ...
        <Text style={styles.doneBody}>
          Head to Settings any time to update your pricing defaults, payment processor, or AI
          assistant keys.
        </Text>
      )}
    </View>
