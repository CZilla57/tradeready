...
export default function AddInvoiceScreen({ route, navigation }: { route: any; navigation: any }) {
  ...
  useEffect(() => {
    ...
    if (isEditing) {
      loadInvoices().then((invoices: Invoice[]) => {
        const inv = invoices.find((i) => i.id === invoiceId);
        if (inv) {
          setCustomer((inv as any).customer);
          setNumber(inv.number);
          setAmount(String((inv as any).amount));
          setDue(inv.due);
          setEmail((inv as any).email);
          setPhone((inv as any).phone);
          setDesc((inv as any).desc);
        }
      });
    } else if (prefill) {
