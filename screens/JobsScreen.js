      ...
      >
        <View style={styles.cardTop}>
          ...
          <View style={{ alignItems: "flex-end", gap: 5 }}>
            {job.estimateTotal > 0 && (
              <Text style={styles.jobAmount}>{formatCurrency(job.estimateTotal)}</Text>
            )}
  ...
  return (
    <SafeAreaView style={styles.container} edges={["bottom"]}>
      ...
      <View style={styles.statsRow}>
        ...
        <View style={{ width: spacing.sm }} />
        <StatCard label="Pending value" value={formatCurrency(pendingValue)} valueColor={colors.accent} />
      </View>
...
function QuickAction({ job, navigation }) {
  const actions = {
