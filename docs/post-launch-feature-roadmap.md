...
**Created:** 2026-07-17
**Status:** Backlog / planning. **None of this blocks the iOS App Store submission.** Do not start any of it until the review clears and the owner gives a go-ahead per the phase-gate rule (`tradeready-change-control`).
...
| # | Feature | Impact | Effort | Reuses |
|---|---------|--------|--------|--------|
| 1 | Estimate approval loop (+ e-sign) | 🔥🔥🔥 | Med | SendEstimateScreen, status pipeline, hosted legal site |
| 2 | Appointment & "on my way" reminders | 🔥🔥🔥 | Low–Med | Resend outreach + cron, notifications |
| 3 | Deposits / partial payments | 🔥🔥 | Med | Stripe Connect, invoice model |
| 4 | Tax set-aside / quarterly estimate | 🔥🔥🔥 (differentiator) | Med | P&L data, mileage, AI coach |
| 5 | Receipt OCR | 🔥🔥 | Med | Photo pipeline, backend AI proxy |
| 6 | Recurring invoices (maintenance plans) | 🔥 | Med | RecurringJobs engine, invoice model |
| 7 | Accounting / CSV export | 🔥 | Low | Existing money/expense data |
| 8 | Automatic (GPS) mileage | 🔥🔥 | High (native) | MileageLog / Trip model |
| 9 | Online booking / request-a-quote link | 🔥🔥🔥 (new-work ceiling) | High (web) | Sync write path, Jobs list |
| 10 | Two-way SMS inbox | 🔥 | High | Outreach infra — evaluate before committing |
