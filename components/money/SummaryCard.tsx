...
function changePct(current: number, prev: number | null): number | null {
  if (prev === null || prev === 0) return null;
  return Math.round(((current - prev) / Math.abs(prev)) * 100);
}

function ChangeLabel({ pct, inverse, colors }: { pct: number | null; inverse: boolean; colors: ColorScheme }) {
        ...
        <View style={styles.marginBarContainer}>
          <View style={styles.marginBarBg}>
            <View style={[
              ...
              {
                width: `${Math.max(0, Math.min(100, (profit / income) * 100))}%` as `${number}%`,
